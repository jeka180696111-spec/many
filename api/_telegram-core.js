...
return async function handler(req, res) {
  ...
  try {
    ...
    // У JS regex \b працює тільки для ASCII — для кирилиці не спрацьовує.
    // Тому використовуємо lookaround з явним списком "буквених" символів,
    // щоб не ловити "финал", "финиш" як звертання до Фінна.
    const CYR_LET = '[А-Яа-яЁёІіЇїЄєҐґA-Za-z]';
    const mentionsFinn = new RegExp(
      `(?<!${CYR_LET})(финн?|фінн?|finn|казначей)(?!${CYR_LET})|@finn?`,
      'i'
    ).test(text);
    const isBabyFood = /(ч\.?\s*л\.?|ст\.?\s*л\.?|чайн\w*\s+ложк|столов\w*\s+ложк|пюре|прикорм|кабач|брокколи|цветн\w*\s+капуст|банан|тыкв|каш\w*\s+(дет|малыш)|грудь\s+[ЛП]|молочн\w*\s+смес|памперс|подгузник|уснул|проснулся|какал|мокрый)/i.test(text);
    ...
    if (isFamilyHQGroup && !mentionsFinn) {
      const hasMoneySignal = /(грн|₴|гривен|гривень|\$|usd|eur|€|руб|rub|доллар|долар|евро|євро)/i.test(text)
        || /(купил|купила|купили|потратил|потратила|потратили|витрат|заплат|зарплат|премия|премія|доход|доходи|расход|чек|цена|вартість|баланс|бюджет|сводк|зведенн|отчет|звіт)/i.test(text)
        || /(^|\s)зп(\s|$|,|\.|!|\?)/i.test(text);
      if (!hasMoneySignal) {
        return res.status(200).json({ ok: true });
      }
